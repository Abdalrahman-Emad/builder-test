...
// @ts-ignore: Side-effect CSS import for grapesjs styles
...
export interface UseTemplateBuilderResult {
  editorContainerId: string;
  blocksContainerId: string;
  styleManagerContainerId: string;
  state: TemplateBuilderState;
  patch: (partial: Partial<TemplateBuilderState>) => void;
  allVariables: TemplateVariable[];
  previewHtml: string;
  mountEditor: () => () => void;
  save: (templateKey: string, templateName: string) => Promise<boolean>;
  undo: () => void;
  redo: () => void;
  ...
  addCustomVariable: (key: string, label: string, example?: string) => boolean;
  insertVariableAtCursor: (variable: TemplateVariable) => void;
}
...
export function useTemplateBuilder({
  initialPayload,
   onSave,
   onExportHtml,
}: UseTemplateBuilderOptions = {}): UseTemplateBuilderResult {
  ...
  const save = useCallback(
    async (templateKey: string, templateName: string) => {
      ...
      try {
        ...
        const html = buildExportHTML(rawHtml, editor.getCss() ?? "")
